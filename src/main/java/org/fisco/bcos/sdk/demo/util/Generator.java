...
public class Generator {
     ...
     */
    public static int[][][] generateTransactionTestCases(int total, Integer conflictRate, Integer groups){
        gi = 0;
     ...
     * @return 冲突交易数组
     */
    private static void generateConflictTransactionTestCases(int total, Integer conflictRate, Integer groups, int[][][] transactions) {
...
//        int[][][] conflictTransactionGroups = new int[groups][][];
        for(int i = 0; i < groups; i ++) {
            ...
            if(i == groups - 1)
                // 最后一组需要特判
                transactions[i] = generateConflictTransactionTestCasesPerGroup(conflictTransactionsPerGroup - lastGroupToRemove);
            else
                transactions[i] = generateConflictTransactionTestCasesPerGroup(conflictTransactionsPerGroup);
            System.out.println();
    ...
    public static int[][] generateConflictTransactionTestCasesPerGroup(int total) {
        // a, b, c 为根据等差数列求和公式构造的一元二次方程参数，设层级为 n，Sn 为到第 n 层的累加和, d 为公差，Sn = n * a1 + n * (n - 1) * d / 2
        ...
        for(int i = layers - 1; i >= 1; i --){
            for(int j = 0; j < i; j ++) {
                ...
                conflictTransactionContainer[i - 1][j][1] = conflictTransactionContainer[i][j + 1][1];
            }
        }
     ...
     * @return 非冲突交易数组
     */
    private static void generateNonConflictTransactionTestCases(int totalOfNonConflictTransactions, int[][][] transactions) {
